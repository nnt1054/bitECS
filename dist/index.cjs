...
var canonicalize = (target) => {
  ...
  const dirtyProps = target.filter(isDirtyModifier).map(fromModifierToComponent).filter(isProperty);
  const dirtyComponentProps = dirtyProps.map((prop) => prop[$storeBase]()).map(storeFlattened).reduce(concat, []).filter((prop) => !dirtyProps.includes(prop));
...
var defineSerializer = (target, maxBytes = 2e7) => {
  ...
  return (ents) => {
    ...
    for (let pid = 0; pid < dirtyProps.length; pid++) {
      ...
      if (!dirtyCache.has(component))
        dirtyCache.set(component, /* @__PURE__ */ new Map());
